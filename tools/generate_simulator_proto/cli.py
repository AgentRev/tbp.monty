...
def _is_optional(t: Any) -> Tuple[bool, Any]:
    ...
    if origin is Union:
        ...
        if len(non_none) == 1 and len(args) == 2 and any(a is type(None) for a in args):
        ...
        try:
            ...
            try:
                ...
                if result.returncode != 0:
                    ...
                    print(
                        f"Warning: protoc failed, using fallback parsing: {result.stderr}"
                    )
                    ...
                    if len(parts) == 2:
                        ...
                        if field_words:
                            ...
                            try:
                                ...
                                self._field_numbers[current_message][field_name] = (
                                    field_number
                                )
                            except ValueError:
...
def _fields_from_parameters(params: List[inspect.Parameter]) -> List[FieldSpec]:
    ...
    for p in params:
        ...
        t = p.annotation if p.annotation is not inspect._empty else bytes
        # Optional?
